...
``.[full]`` 是默认的端到端组合，包含 openpi 的 Pi0.5 VLA、LIBERO-PRO 仿真器
与 SAM 3.0，跑在 RLinf 运行时之上。
...
只要跑通，就说明 env server、VLA server、SAM3 server 和推理大脑四者都正常。
...
这些日志都放在本次运行的临时目录下，所以每一次失败的运行都是自包含的，排查
起来很方便。
