...
observations, registered tool results, and the allowlisted semantic recipe.
Fresh perception overrides recipe priors for coordinates, pixels, object
identity, arm choice, approach, table height, and scene relations. In randomized
scenes, rebind all geometry and distractors. The recipe never authorizes saved
coordinates, object poses, or historical action traces. The current native task
language is authoritative for target color, object identity, and goal relation;
conflicting historical recipe attributes must be ignored."""
...
the exact view whose RGB supplied the pixel or bbox. head pixels belong to head
world_xyz, and head_hi pixels belong to head_hi world_xyz. Never pass
high-resolution coordinates to a base-resolution view."""
...
target globally, stage safely, then refine locally. High-resolution views may
be overexposed; fall back to the matching base-resolution view when needed."""
...
WORKFLOW = [
    "Inspect the initial registered state and head view.",
...
A VLA chunk, planned path, grasp, plausible placement, recipe completion, or
...
USER_MODE = """Solve one no-restart episode unless controlled reset was explicitly
enabled. Treat semantic recipe content as a clean-scene prior and all current
